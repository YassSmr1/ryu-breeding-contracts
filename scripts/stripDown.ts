...
async function main() {
  ...
  rl.question(
    "On wich token do u want to use stripdown potions ? ",
    async (firstToken: string) => {
      rl.close();
      const tx = await labs.connect(owner).stripDown(parseInt(firstToken));
      const receipt = await tx.wait();

      if (receipt.status && receipt.status === 1) {
        console.log(
          "Successfully used stripdown potions on this token",
          firstToken
        );
      } else {
