...
describe("Labs: Fraternal Twins Potions", function () {
  ...
  it("User 1 should pay 7500 $nryu for fraternal twins function", async function () {
    const cost = ethers.utils.parseEther("7500");
    const balanceBefore = await mockErc20.balanceOf(user1.address);

    await labs.connect(user1).fraternalTwins(gen2tokenUser1[0]);

    const balanceAfter = await mockErc20.balanceOf(user1.address);
    expect(balanceAfter).to.be.equal(balanceBefore.sub(cost));
  });

  it("Should burn 4500 $nryu for function usage", async function () {
    const burnedAmount = ethers.utils.parseEther("4500");
  ...
  it("Should send 3000 $nryu for function usage to treasury wallet", async function () {
    const expected = ethers.utils.parseEther("3000");
  ...
  it("Should be not allowed to use fraternal twins on an nft that i dont own", async function () {
    await expect(
      labs.connect(user1).fraternalTwins(gen2tokenOwner[0])
    ).to.be.revertedWith("Owner only endpoint");
  });

  it("Should be not allowed to use fraternal twins twice on an nft with same parents", async function () {
    await labs.connect(user1).fraternalTwins(gen2tokenUser1[0]);
    await expect(
      labs.connect(user1).fraternalTwins(gen2tokenUser1[1])
    ).to.be.revertedWith("Already used on these parents");
  });

  it("Should be not allowed to use fraternal twins twice on the same nft", async function () {
    await labs.connect(user1).fraternalTwins(gen2tokenUser1[0]);
    await expect(
      labs.connect(user1).fraternalTwins(gen2tokenUser1[0])
    ).to.be.revertedWith("Already used on these parents");
  });

  it("Should correctly display fraternal as non usuable", async function () {
    await labs.connect(user1).fraternalTwins(gen2tokenUser1[0]);
    const usages = await labs.getPotionsUsageForToken(gen2tokenUser1[0]);
    expect(usages[0]).to.equal(false);
  });
  ...
  it("Should emit an event with Breed(user1.address, parentMale, parentFemale, gen2newId)", async function () {
    const parentMale = await gen2.tokenIdToParents(gen2tokenUser1[0], 0);
    const parentFemale = await gen2.tokenIdToParents(gen2tokenUser1[0], 1);
    await expect(labs.connect(user1).fraternalTwins(gen2tokenUser1[0]))
      .to.emit(gen2, "Breed")
      .withArgs(user1.address, parentMale, parentFemale, gen2newId);
  });
});
