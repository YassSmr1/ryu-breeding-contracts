...
describe("Labs: Identical Twins Potions", function () {
  ...
  beforeEach(async () => {
    ...
    gen2newId = gen2tokenUser1[gen2tokenUser1.length - 1].toNumber() + 1;
  });

  it("User 1 should pay 15000 $nryu for Identical twins function", async function () {
    const cost = ethers.utils.parseEther("15000");
    const balanceBefore = await mockErc20.balanceOf(user1.address);

    await labs.connect(user1).identicalTwins(gen2tokenUser1[0]);

    const balanceAfter = await mockErc20.balanceOf(user1.address);
    expect(balanceAfter).to.be.equal(balanceBefore.sub(cost));
  });

  it("Should burn 9000 $nryu for Identical Twins function usage", async function () {
    const burnedAmount = ethers.utils.parseEther("9000");
  ...
  it("Should send 6000 $nryu for Identitcal Twins function usage to treasury wallet", async function () {
    const expected = ethers.utils.parseEther("6000");
  ...
  it("Should be not allowed to use identitcal twins on an nft that i dont own", async function () {
    await expect(
      labs.connect(user1).identicalTwins(gen2tokenOwner[0])
    ).to.be.revertedWith("Owner only endpoint");
  });

  it("Should be not allowed to use identicalTwins twice on an nft with same parents", async function () {
    await labs.connect(user1).identicalTwins(gen2tokenUser1[0]);
    await expect(
      labs.connect(user1).identicalTwins(gen2tokenUser1[1])
    ).to.be.revertedWith("Already used on these parents");
  });

  it("Should be not allowed to use identicalTwins twice on the same nft", async function () {
    await labs.connect(user1).identicalTwins(gen2tokenUser1[0]);
    await expect(
      labs.connect(user1).identicalTwins(gen2tokenUser1[0])
    ).to.be.revertedWith("Already used on these parents");
  });

  it("Should correctly display identical as non usuable", async function () {
    await labs.connect(user1).identicalTwins(gen2tokenUser1[0]);
    const usages = await labs.getPotionsUsageForToken(gen2tokenUser1[0]);
    expect(usages[2]).to.equal(false);
  });

  it("Should save the parents of new nft", async function () {
    const parentMale = await gen2.tokenIdToParents(gen2tokenUser1[0], 0);
    const parentFemale = await gen2.tokenIdToParents(gen2tokenUser1[0], 1);

    await labs.connect(user1).identicalTwins(gen2tokenUser1[0]);

    expect(await gen2.tokenIdToParents(gen2newId, 0)).to.be.equal(parentMale);
    expect(await gen2.tokenIdToParents(gen2newId, 1)).to.be.equal(parentFemale);
  });
  ...
  it("Should be used only on hacthed eggs", async function () {
    await expect(
      labs
        .connect(user1)
        .identicalTwins(gen2tokenUser1[gen2tokenUser1.length - 1])
    ).to.be.revertedWith("Only on hatched eggs");
  });

  it("Should emit an event with Identical(user1.address, twinsId, gen2newId)", async function () {
    await expect(labs.connect(user1).identicalTwins(gen2tokenUser1[0]))
      .to.emit(gen2, "Identical")
      .withArgs(user1.address, gen2tokenUser1[0], gen2newId);
  });
});
